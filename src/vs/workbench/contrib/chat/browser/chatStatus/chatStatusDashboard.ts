	...
	private render(): void {
		...
		if (chatQuota || completionsQuota || premiumChatQuota) {
			...
			if (this.chatEntitlementService.entitlement === ChatEntitlement.Free && (Number(chatQuota?.percentRemaining) <= 25 || Number(completionsQuota?.percentRemaining) <= 25)) {
				...
				upgradeProButton.label = localize('upgradeToCopilotPro', "Upgrade to Forge Pro");
		...
		// Anonymous Indicator
			...
			addSeparator(localize('anonymousTitle', "Forge Usage"));
		...
		// New to Chat / Signed out
		{
			...
			if (newUser || signedOut || disabled) {
				...
				if (newUser && anonymousUser) {
					...
					descriptionText = localize('activateDescription', "Set up Forge to use AI features.");
				} else if (anonymousUser) {
					descriptionText = localize('enableMoreDescription', "Sign in to enable more Forge AI features.");
				} else if (disabled) {
					descriptionText = localize('enableDescription', "Enable Forge to use AI features.");
				} else {
					descriptionText = localize('signInDescription', "Sign in to use Forge AI features.");
				}
